...
export const usePreAppointments = (locationUuid: string, yearWeek: any, successCode?: any) => {
  let url = `/ws/rest/v1/amrs/preappointment?locationUUID=${locationUuid}&yearWeek=${yearWeek?.id}`;
  if (successCode.id !== '' && successCode) {
    url += successCode.id;
  }

  //const patientFlagsUrl = `/ws/rest/v1/amrs/preappointment?locationUuids=${locationUuid}&yearWeek=${yearWeek?.id}`;
