...
export const Admissionqueue: React.FC = () => {
    ...
    return paginatedResults.map((entry) => ({
      ...
      action: (
        <OverflowMenu flipped={document?.dir === 'rtl'} aria-label="overflow-menu">
          <OverflowMenuItem itemText="Admit body" />
  ...
  const tableColumns = [
    ...
    { id: 2, header: t('date', 'DATE AND TIME OF DEATH'), key: 'date' },
    { id: 3, header: t('cause', 'CAUSE OF DEATH'), key: 'cause' },
    { id: 4, header: t('created', 'CREATED BY'), key: 'created' },
    { id: 5, header: t('date-created', 'DATE CREATED'), key: 'date-created' },
    { id: 6, header: t('action', 'ACTION'), key: 'action' },
  ];
